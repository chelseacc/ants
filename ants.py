        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container:
                    ...
                    insect.place = None
                elif hasattr(self.ant, 'Queen') and self.ant.Queen:
                    self.ant = self.ant
                else:
                    self.ant = None
                    insect.place = None
                ...
                if hasattr(self.ant, 'container') and self.ant.container:
                    if hasattr(self.ant.ant, 'Queen') and self.ant.ant.Queen:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        i = 0
        start = self.place
        while start.entrance != None and start != hive:
            if i >= self.min_range and i <= self.max_range and len(start.bees) > 0:
    ...
    def action(self, colony):
        ...
        if self.Queen:
            ...
            while behind is not None:
                if behind.ant:
                    if behind.ant not in self.doubled_ants:
                        self.doubled_ants.append(behind.ant)
                        if behind.ant.container and behind.ant.ant and behind.ant.ant not in self.doubled_ants:
                            behind.ant.ant.damage *= 2
                            self.doubled_ants.append(behind.ant.ant)
                        behind.ant.damage *= 2
                    elif behind.ant.container and behind.ant.ant and behind.ant.ant not in self.doubled_ants:
                        behind.ant.ant.damage *= 2
                        self.doubled_ants.append(behind.ant.ant)
                behind = behind.exit
        else:
